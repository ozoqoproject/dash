...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //pozoqo-tx using stdin
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty pozoqo.conf if it does not exist
...
//    std::string strCopyrightHolders = strPrefix + strprintf(" %u-%u ", nStartYear, nEndYear) + strprintf(_(COPYRIGHT_HOLDERS).translated, _(COPYRIGHT_HOLDERS_SUBSTITUTION));
    ...
    // Check for untranslated substitution to make sure Pozoqo Core copyright is not removed by accident
    if (copyright_devs.find("Pozoqo Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2022, nEndYear) + "The Pozoqo Core developers";
    }
    // Check for untranslated substitution to make sure Pozoqo Core copyright is not removed by accident
