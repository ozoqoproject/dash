                                     ...
                                     const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (llmq::quorumInstantSendManager->IsWaitingForTx(hash)) {
        ...
        for (const CTxIn &txin : tx.vin)
        {
            ...
            if (ptxConflicting)
            {
                // Transaction conflicts with mempool and RBF doesn't exist in Pozoqo
...
{
    double nSubsidy = 10000;      // (declaring the reward variable and its original/default amount)
...
static int64_t nTimePozoqoSpecific = 0;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, *m_quorum_block_processor, state, view, fJustCheck, fScriptChecks)) {
        return error("ConnectBlock(PZQ): ProcessSpecialTxsInBlock for block %s failed with %s",
    ...
    // PZQ : CHECK TRANSACTIONS FOR INSTANTSEND
        ...
        for (const auto& tx : block.vtx) {
            ...
            while (llmq::CInstantSendLockPtr conflictLock = m_isman->GetConflictingLock(*tx)) {
                if (m_clhandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                    LogPrint(BCLog::ALL, "ConnectBlock(PZQ): chain-locked transaction %s overrides islock %s\n",
                    ...
                    return state.Invalid(ValidationInvalidReason::TX_CONFLICT_LOCK, error("ConnectBlock(PZQ): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), conflictLock->txid.ToString()), REJECT_INVALID, "conflict-tx-lock");
                }
            }
        }
    }
    ...
    // PZQ : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(*sporkManager, *governance, block, pindex->nHeight, blockReward, strError)) {
        ...
        return state.Invalid(ValidationInvalidReason::NONE, error("ConnectBlock(PZQ): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    ...
    if (!IsBlockPayeeValid(*sporkManager, *governance, *block.vtx[0], pindex->nHeight, blockReward)) {
        // NOTE: Do not punish, the node might be missing governance data
        return state.Invalid(ValidationInvalidReason::NONE, error("ConnectBlock(PZQ): couldn't find masternode or superblock payments"), REJECT_INVALID, "bad-cb-payee");
    }
    ...
    int64_t nTime5 = GetTimeMicros(); nTimePozoqoSpecific += nTime5 - nTime4;
    LogPrint(BCLog::BENCHMARK, "    - Pozoqo specific: %.2fms [%.2fs (%.2fms/blk)]\n", MILLI * (nTime5 - nTime4), nTimePozoqoSpecific * MICRO, nTimePozoqoSpecific * MILLI / nBlocksTotal);
...
bool CChainState::RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs, const CChainParams& params)
{
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, *m_quorum_block_processor, state, inputs, false /*fJustCheck*/, false /*fScriptChecks*/)) {
        return error("RollforwardBlock(PZQ): ProcessSpecialTxsInBlock for block %s failed with %s",
            pindex->GetBlockHash().ToString(), FormatStateMessage(state));
    }
