...
void SetupServerArgs(NodeContext& node)
{
    ...
    argsman.AddArg("-sporkaddr=<pozoqoaddress>", "Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.", ArgsManager::ALLOW_ANY, OptionsCategory::DEBUG_TEST);
    ...
    argsman.AddArg("-platform-user=<user>", "Set the username for the \"platform user\", a restricted user intended to be used by Pozoqo Platform, to the specified username.", ArgsManager::ALLOW_ANY, OptionsCategory::MASTERNODE);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/ozoqoproject/pozoqo>";
    const std::string URL_WEBSITE = "<https://pozoqo.tech>";
...
/** Sanity checks
 *  Ensure that Pozoqo Core is running in a usable environment with all
...
{
    // Make sure only a single Pozoqo Core process is using the data directory.
...
bool AppInitMain(const CoreContext& context, NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.IsArgSet("-datadir") && !fs::path(args.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if Pozoqo Core is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if Pozoqo Core is started while in a temporary directory.\n",
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                if (!fDisableGovernance && !args.GetBoolArg("-txindex", DEFAULT_TXINDEX) && chainparams.NetworkIDString() != CBaseChainParams::REGTEST) { // TODO remove this when pruning is fixed. See https://github.com/pozoqopay/pozoqo/pull/1817 and https://github.com/pozoqopay/pozoqo/pull/1743
    ...
    // ********************************************************* Step 10b: schedule Pozoqo-specific tasks
