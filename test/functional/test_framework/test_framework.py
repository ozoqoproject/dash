...
TMPDIR_PREFIX = "pozoqo_func_test_"
...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def parse_args(self):
        ...
        parser.add_argument("--pozoqod-arg", dest="pozoqod_extra_args", default=[], action="append",
                            help="Pass extra args to all pozoqod instances")
    ...
    def setup(self):
        ...
        self.config = config
        self.options.bitcoind = os.getenv("BITCOIND", default=config["environment"]["BUILDDIR"] + '/src/pozoqod' + config["environment"]["EXEEXT"])
        ...
        self.extra_args_from_options = self.options.pozoqod_extra_args
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain)  # Overwrite port/rpcport in pozoqo.conf
    ...
    def skip_if_no_bitcoind_zmq(self):
        """Skip the running test if pozoqod has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("pozoqod has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if pozoqo-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("pozoqo-wallet has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if pozoqo-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("pozoqo-cli has not been compiled.")

    def is_cli_compiled(self):
        """Checks whether pozoqo-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether pozoqo-wallet was compiled."""
...
class DashTestFramework(BitcoinTestFramework):
    ...
    def set_pozoqo_test_params(self, num_nodes, masterodes_count, extra_args=None, fast_dip3_enforcement=False):
        ...
        self.set_pozoqo_dip8_activation(200)

        # LLMQ default test params (no need to pass -llmqtestparams)
        self.llmq_size = 3
        self.llmq_threshold = 2

        # This is nRequestTimeout in pozoqo-q-recovery thread
    ...
    def set_pozoqo_dip8_activation(self, activate_after_block):
    ...
    def set_pozoqo_llmq_test_params(self, llmq_size, llmq_threshold):
