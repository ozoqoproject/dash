...
- [Development guidelines](#development-guidelines)
    - [General Pozoqo Core](#general-pozoqo-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Pozoqo Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Pozoqo Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/ozoqoproject/pozoqo/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_pozoqo
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_pozoqo --log_level=test_suite
$ valgrind -v --leak-check=full src/pozoqod -printtoconsole
...
# A coverage report will now be accessible at `./test_pozoqo.coverage/index.html`.
```
...
To profile a running pozoqod process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep pozoqod` -- sleep 60
```
...
Pozoqo Core can be compiled with various "sanitizers" enabled, which add
...
- ThreadSocketHandler : Sends/Receives data from peers on port 18181.
...
However, in open source software such as Pozoqo Core, where everyone uses
...
pay attention to for reviewers of Pozoqo Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Pozoqo Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
be sent upstream, but bugfixes may also be prudent to PR against Pozoqo Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `pozoqo-cli`, and can be surprising to users.
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Pozoqo Core, the wallet was always
