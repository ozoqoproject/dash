...
*Setup instructions for a Gitian build of Pozoqo Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Pozoqo
...
- [Building Pozoqo Core](#building-pozoqo-core)
...
Clone the git repositories for Pozoqo Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/ozoqoproject/pozoqo
...
in the Pozoqo Core repository under 'Fetch and create inputs' to install sources which require
...
To build Pozoqo Core (for Linux, macOS and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#setup-and-perform-gitian-builds) in the Pozoqo Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/pozoqo/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/ozoqoproject/pozoqo
...
For example:
```bash
URL=https://github.com/crowning-/pozoqo.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit pozoqo=${COMMIT} --url pozoqo=${URL} ../pozoqo/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit pozoqo=${COMMIT} --url pozoqo=${URL} ../pozoqo/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit pozoqo=${COMMIT} --url pozoqo=${URL} ../pozoqo/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the pozoqo git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../pozoqo/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/pozoqo
SIGPATH=/some/root/path/pozoqo-detached-sigs

./bin/gbuild --url pozoqo=${BTCPATH},signature=${SIGPATH} ../pozoqo/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/pozoqo-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/pozoqo-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/pozoqo-osx-build.assert
```
...
[pozoqo/gitian.sigs](https://github.com/ozoqoproject/gitian.sigs/) repository, or if that's not possible to create a pull
request.
